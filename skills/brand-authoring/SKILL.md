...
- `layout`: `header_style`, `title_align`, `title_case`, `heading_weight`, `body_weight`, `pptx_heading_scale` (an optional editable-output calibration), `radius`, `logo_variant`, `background_image_opacity`, and (for light/dark artwork) `image_text_color`;
