...
The report surface remains a single canonical A4 PDF with selectable text; the
temporary report PNG/PPTX adapter experiment was removed from the public
contract. Slide output now has three explicit surfaces: a selectable-text PDF
generated from the shared SVG composition, PNG pages rasterized from that PDF
for LLM visual QA, and an editable PPTX generated by the existing native
adapter. LibreOffice is used only by optional PPTX round-trip QA, never by the
runtime renderer. Slide output continues to use its existing image slots
(charts/backgrounds), not arbitrary report content nodes. M6 is complete for
the source/image contract and future seams; release metadata remains on the
pre-release line until the final release gate.
